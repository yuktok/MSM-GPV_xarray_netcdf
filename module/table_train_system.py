...
def train(
    ...
    for i, (x_i, y_i, table_i) in enumerate(train_loader):
        x_i = x_i.to(DEVICE)
        y_i = y_i.to(DEVICE).reshape(-1, 1).float()
        table_i = table_i.to(DEVICE).float()
        
        output = model(x_i, table_i)
...
def predict(model: nn.Module, loader:  torch.utils.data.DataLoader) -> np.ndarray:
    ...
    for x_i, y_i, table_i in loader:

        # 明示的に勾配を計算しないように指定することができます. 
        # この関数ではモデルの更新はせずに単に出力だけを使いますので勾配は不要です.

        with torch.no_grad():
            output = model(x_i.to(DEVICE) ,table_i.to(DEVICE).float())
